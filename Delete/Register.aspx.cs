...
namespace StudentRegistration
{
    public partial class Register : System.Web.UI.Page
    {
        ...
        /*protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                foreach (GridViewRow row in GridView1.Rows)
                {
                    ...
                    {
                        int deletedRowIndex = row.RowIndex;
                        ...
                        using (SqlConnection _connection = new SqlConnection(ConfigurationManager.ConnectionStrings["dbconnection"].ConnectionString))
                        {
                            ...
                            int idToDelete = Convert.ToInt32(row.Cells[1].Text);
                            ...
                            string updateIdsQuery = "UPDATE studreg SET ID=ID-1 WHERE ID>@DeletedId";
                            using (SqlCommand cmd = new SqlCommand(updateIdsQuery, _connection))
                            {
                                cmd.Parameters.AddWithValue("@DeletedId", idToDelete);
                                cmd.ExecuteNonQuery();
                            }
                        }
                    }
                ...
                Response.Write("<script>alert('Selected student(s) deleted successfully!')</script>");
            }
            catch (Exception ex)
            {
                Response.Write("Error: " + ex.ToString());
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                // Add the onclick attribute to each data row
                ...
                e.Row.Cells[0].Controls.Add(chkSelect);
            }
        }
        protected void GridView1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (GridView1.SelectedIndex >= 0 && GridView1.SelectedIndex < GridView1.Rows.Count)
                {
                    ...
                    // Example: Display the selected row's ID
                    string selectedRowId = selectedRow.Cells[0].Text;
                    Response.Write("Selected Row ID: " + selectedRowId);

                    // You can perform additional actions with the selected row as needed
                }
            }
            catch (Exception ex)
            {
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (GridViewRow row in GridView1.Rows)
                {
                    ...
                    if (chkBox != null && chkBox.Checked)
                    {
                        int idToDelete = Convert.ToInt32(row.Cells[0].Text); // Assuming ID is in the first column
        ...
        private void DeleteSelectedRows(List<int> deletedIds)
        {
            using (SqlConnection _connection = new SqlConnection(ConfigurationManager.ConnectionStrings["dbconnection"].ConnectionString))
            {
                ...
                // Update the IDs in the database
                UpdateIdsInDatabase(deletedIds);
            }
        }
        private void UpdateIdsInDatabase(List<int> deletedIds)
        {
            using (SqlConnection _connection = new SqlConnection(ConfigurationManager.ConnectionStrings["dbconnection"].ConnectionString))
            {
                _connection.Open();
